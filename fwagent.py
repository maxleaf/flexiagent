...
class FwAgent(FwObject):
    ...
    def _mark_connection_failure(self, err):
        try:
            with open(fwglobals.g.CONN_FAILURE_FILE, 'w') as f:
                ...
                self.log_debug("_mark_connection_failure: %s" % str(err))
        except Exception as e:
            self.log_excep("Failed to create connection failure file: %s" % str(e))

    def _clean_connection_failure(self):
        if os.path.exists(fwglobals.g.CONN_FAILURE_FILE):
            os.remove(fwglobals.g.CONN_FAILURE_FILE)
            self.log_debug("_clean_connection_failure")

    def _setup_repository(self, repo):
        # Extract repo info. e.g. 'https://deb.flexiwan.com|flexiWAN|main'
        repo_split = repo.split('|')
        if len(repo_split) != 3:
            self.log_error("Registration error: Incorrect repository info %s" % (repo))
            return False
        ...
        if len(repo_files) != 1:
            self.log_error("Registration error: Folder %s must include a single repository file, found %d" %
        ...
        repo_match = re.match(r'^deb[ \t]+\[[ \t]+arch=(.+)[ \t]+\][ \t]+(http.*)/(.+)[ \t]+(.+)[ \t]+(.+)$',
            repo_config)
        if not repo_match:
            self.log_error("Registration error: repository configuration can't be parsed. File=%s, Config=%s" %
        ...
        if (found_server != repo_server or found_repo != repo_repo or found_name != repo_name):
            ...
            self.log_info("Overwriting repository from token, token_repo=%s, prev_repo_config=%s, new_repo_config=%s" %
    ...
    def _decode_token_and_setup_environment(self, token):
        ...
        if server:
            ...
            self.log_info("Using management url from token: %s" % (server))
    ...
    def register(self):
        ...
        self.log_info("registering with flexiManage...")

        self.register_error = ''

        if not loadsimulator.g.enabled():
            if os.path.exists(fwglobals.g.DEVICE_TOKEN_FILE):
                self.log_info("register: already registered, to refresh run 'fwagent reset' and retry")
                return True
        ...
        try:
            ...
            self.log_excep("Failed to decode and setup environment: %s (%s)" %(str(e), traceback.format_exc()))
            return False

        if fwutils.vpp_does_run():
            self.log_error("register: router is running, it by 'fwagent stop' and retry by 'fwagent start'")
            return False
        ...
        if machine_id == None:
            self.log_error("register: get_machine_id failed, make sure you're running in sudo privileges")
            return False
        ...
        self.log_debug("Registering to %s with: %s" % (url, json.dumps(data)))
        ...
        try:
            ...
            self.log_info("Registation successful with parameters:")
            self.log_info("  Hostname:  " + machine_name)
            self.log_info("  IP List:   " + ip_list)
            self.log_info("  Device ID: " + machine_id)
            self.log_info("Run connect after approving device in flexiManage")

        except uerr.URLError as e:
            if hasattr(e, 'code'):
                server_response = e.read().decode()
                latestVersion = e.headers.get('latestVersion','None')
                self.log_error('register: got %s - %s' % (str(e.code), hsvr.BaseHTTPRequestHandler.responses[e.code][0]))
                self.log_error('register: Server response: %s' % server_response)
                self.log_error('latestVersion: %s' % (latestVersion))
                try:
                ...
                if e.code == 403: # version too low, try to upgrade immediately
                    self.log_error('Trying device auto upgrade...')
                    fwglobals.g.handle_request({'message':'upgrade-device-sw','params':{'version':latestVersion}})
            elif hasattr(e, 'reason'):
                self.log_error('register: failed to connect to %s: %s' % (fwglobals.g.cfg.MANAGEMENT_URL, e.reason))
            return False
        except:
            self.log_error('register: failed to send request to server %s: %s' % \
    ...
    def websocket_thread(self, url, header_UserAgent, id):
        ...
        while loadsimulator.g.started:
            ...
            self.log_info("websocket_thread %d: retry connection in %d seconds" % (id, retry_sec))
    ...
    def connect(self):
        ...
        :returns: `True` if connection was established and than was closed gracefully,
                  `False` otherwise.
        """
        self.log_info("connecting to flexiManage...")
        ...
        if not os.path.exists(device_token_fname):
            self.log_error("connect: device token not found (" + device_token_fname + "), please register first")
            return False
        try:
            ...
            self.log_error("connect: failed to load device token from " + device_token_fname + ": " + format())
            return False
        ...
        machine_id = fwutils.get_machine_id()
        if machine_id == None:
            self.log_error("connect: can't connect (failed to retrieve machine ID in fwutils.py:get_machine_id")
            return False
        ...
        if self.connection_error_code:
            ...
            return False
        self.log_info("connection to flexiManage was closed")
        return True
    ...
    def reconnect(self):
        ...
        """
        if self.ws == None:
            self.log_info("flexiManage is not connected, ignore reconnection request")
        elif self.reconnecting:
            self.log_info("reconnection to flexiManage was initiated already")
        else:
            self.log_info("initiate reconnection to flexiManage")
    ...
    def _on_error(self, ws, error):
        ...
        self.log_error("_on_error: connection got error '%s'" % self.connection_error_msg)
    ...
    def _on_close(self, ws):
        ...
        :returns: None.
        """
        self.log_info("_on_close: connection to flexiManage is closed")
        if self.thread_statistics:
    ...
    def _on_open(self, ws):
        ...
        :returns: None.
        """
        self.log_info("connected to flexiManage")
        ...
        if len(self.pending_msg_replies) > 0:
            self.log_info("_on_open: send %d pending replies to flexiManage" % len(self.pending_msg_replies))
            for reply in self.pending_msg_replies:
                self.log_debug("_on_open: sending reply: " + json.dumps(reply))
                ...
                try:  # Ensure thread doesn't exit on exception
                    ...
                    if (slept % timeout) == 0:
                        if self.received_request or self.handling_request:
                            ...
                            self.log_debug("connect: no request was received in %s seconds, drop connection" % timeout)
                            ws.close()
                            self.log_debug("connect: connection was terminated")
                            break
        ...
        if not fwutils.vpp_does_run():
            self.log_info("connect: router is not running, start it in flexiManage")
    ...
    def _on_message(self, ws, message):
        ...
        self.log_debug(seq + " job_id=" + job_id + " request=" + json.dumps(request))
        ...
        self.log_debug(seq + " job_id=" + job_id + " reply=" + json.dumps(reply_str))
        ...
        if self.reconnecting == True:
            self.log_info("_on_message: goes to reestablish connection, queue reply %s" % str(pmsg['seq']))
    ...
    def handle_received_request(self, received_msg):
        ...
        try:
            ...
            self.handling_request = False
             ...
             self.log_error("handle_received_request failed: %s" + str(e))
    ...
    def inject_requests(self, filename, ignore_errors=False, json_requests=None):
        ...
        :returns: N/A.
        """
        self.log_debug("inject_requests(filename=%s, ignore_errors=%s)" % \
...
class FwagentDaemon(FwObject):
    ...
    def _check_system(self):
        ...
        try:
            ...
            self.log_excep("+====================================================")
            self.log_excep("| System checker failed (%d)" % err.returncode)
            self.log_excep("| Fix problems and run 'fwagent start'")
            self.log_excep("| To check and configure system run 'fwsystem_checker'")
            self.log_excep("+=====================================================")
            return False

    def ping(self):
        self.log_debug("ping: alive")
    ...
    def start(self, start_vpp=False, check_system=True):
        ...
        :returns: None.
        """
        self.log_debug("start (start_vpp=%s)" % str(start_vpp))

        if self.active:
            self.log_debug("already started, ignore")
            return
        ...
        if check_system and self._check_system() == False:
            self.log_excep("system checker failed")

        if start_vpp:
            try:
                fwglobals.g.router_api.start_router()
                self.log_debug("vpp started")
            except Exception as e:
                self.log_excep("failed to start vpp: " + str(e))
                return
        self.active  = True
    ...
    def stop(self, stop_router=True):
        ...
        if self.active:
            ...
            self.log_debug("disconnect from server was initiated")
        # Stop vpp ASAP, as no more requests can arrive on connection
        if stop_router:
            try:
                fwglobals.g.handle_request({'message':'stop-router'})
                self.log_debug("router stopped")
            except Exception as e:
                self.log_excep("failed to stop router: " + str(e))
        elif fwglobals.g.router_api.state_is_started():
            self.log_debug("vpp alive, use 'fwagent stop' to stop it")
    ...
    def main(self):
        ...
        :returns: None.
        """
        self.log_info("connection loop was started, use 'fwagent stop' to stop it if needed")
        ...
        while self.active and not self.agent.register():
            ...
            if self.agent.register_error == 'token not found' or \
               self.agent.register_error == 'invalid token':
                self.log_debug('poll %s for modification' % fwglobals.g.cfg.TOKEN_FILE)
            ...
            # If we got same registration reject twice - stop retrials
                ...
                self.log_info("stop registration trials, use 'fwagent start' to resume")
                self.active = False
                return
        ...
        while self.active:
            ...
            if not closed_gracefully and self.active:
                ...
                self.log_info("retry connection in %d seconds" % retry_sec)
                while retry_sec > 0 and self.active:
                    time.sleep(1)   # Check self.active every second to detect Ctrl-C as soon as possible
                    retry_sec -= 1

        self.log_info("connection loop was stopped, use 'fwagent start' to start it again")
