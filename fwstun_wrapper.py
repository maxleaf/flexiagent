...
class FwStunWrap(FwObject):
        ...
        if self.stun_cache:
            for dev_id in self.stun_cache:
                ...
                if self.stun_cache[dev_id].get('local_ip') != '' and \
                    ...
                    self.log_debug(dev_id + ':' + str(self.stun_cache[dev_id]))
    ...
    def initialize(self):
        ...
        if self.standalone:
            return
        self.log_debug("Start sending STUN requests for all WAN interfaces")
        ifaces = fwutils.get_all_interfaces()
        if ifaces:
            ips = [ifaces[dev_id]['addr'] for dev_id in ifaces if ifaces[dev_id]['addr'] != '' \
                                and ifaces[dev_id]['gw'] != '']

            self.log_debug("stun_thread initialize: collected WAN IPs: %s" %(str(ips)))
    ...
    def _update_cache_from_OS(self):
        ...
        self.log_debug("_update_cache_from_OS: WAN IP list from OS %s" %(str(os_addr_list)))
        self.log_debug("_update_cache_from_OS: WAN IP list from STUN cache %s" %(str(cache_ip_list)))
    ...
    def add_addr(self, dev_id, addr, gateway):
        ...
        if dev_id == None:
            ...
            if dev_id == None:
                self.log_debug("add_addr: no dev_id was found for address %s, not updating cache" %(addr))
                return
        ...
        if dev_id not in self.stun_cache or self.stun_cache[dev_id].get('local_ip') != addr.split('/')[0] or \
            ...
            if addr:
                self.log_debug("Updating dev_id address %s IP address %s in Cache" %(dev_id, addr))
            else:
                self.log_debug("Updating dev_id address %s in Cache" %(dev_id))
    ...
    def _handle_stun_none_response(self, dev_id):
        ...
        cached_addr['success'] = False
        cached_addr['server_index'] = 0
        self.log_debug("_handle_stun_none_response: failed getting public IP/port for address %s, retry in %d seconds"\
    ...
    def _handle_stun_response(self, dev_id, p_ip, p_port, nat_type, st_index):
        ...
        if not cached_addr:
            return
        self.log_debug("found external %s:%s for %s:4789" %(p_ip, p_port, cached_addr['local_ip']))
    ...
    def _send_single_stun_request(self, lcl_src_ip, lcl_src_port, stun_idx):
        ...
        self.log_debug("trying to find external %s:%s for device %s" %(lcl_src_ip,lcl_src_port, dev_name))
    ...
    def handle_down_tunnels(self, tunnel_stats):
        ...
        for tunnel in tunnels:
            ...
            if tunnel['src'] not in ips:
                self.log_debug("Tunnel %d is down, but its source address %s was not found in the system"\
            ...
            if tunnel['src'] not in ip_up_set:
                self.log_debug("Tunnel %d is down, updating address %s in STUN interfaces cache"\
                    %(tunnel_id, tunnel['src']))
            ...
            if self.sym_nat_cache.get(dev_id):
                if self.sym_nat_cache[dev_id]['probe_time'] == 0:
                    self.log_debug("Re-try to discover remote edge for tunnel: %d on dev %s"%(tunnel_id, dev_id))
                    self.sym_nat_cache[dev_id]['local_ip'] = tunnel['src']
                    self.sym_nat_cache[dev_id]['probe_time'] = time.time() + 25
            elif dev_id is not None:
                self.log_debug("Try to discover remote edge for tunnel %d on dev %s"%(tunnel_id, dev_id))
                self.sym_nat_cache[dev_id] = {
                            'local_ip'    : tunnel['src'],
                            'probe_time'  : time.time() + 25,
                        }
            else:
                self.log_debug("Dev is %s for tunnel %d"%(dev_id, tunnel_id))
            ...
            if encryption_mode == "ikev2" and role == "responder":
                self.log_debug("ikev2 tunnel %d responder side is down on dev %s"%(tunnel_id, dev_id))
    ...
    def _probe_symmetric_nat(self):
        ...
        for dev_id in list(self.sym_nat_cache):
            ...
            if time.time() >= cached_addr['probe_time']:
                ...
                self.log_debug("Tunnel: discovering remote ip for tunnels with src %s:%s on device %s" \
                    %(src_ip, src_port, dev_name))
                probe_tunnels_dev = fwstun.get_remote_ip_info(src_ip, src_port, dev_name)
                self.log_debug("Tunnel: discovered tunnels %s on dev %s" %(probe_tunnels_dev, dev_name))
            ...
            if stats and stats.get('status') == 'down':
                ...
                if vni in probe_tunnels:
                    if tunnel['dst'] != probe_tunnels[vni]["dst"] or tunnel['dstPort'] != probe_tunnels[vni]["dstPort"]:
                        self.log_debug("Remove tunnel: %s" %(tunnel))
                        ...
                        self.log_debug("Add tunnel: %s" %(tunnel))
