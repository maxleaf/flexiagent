...
class FwagentCli(FwObject):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.log_warning("no daemon Fwagent was found, use local instance")
            self.daemon = None
        ...
        while True:
            try:
                ...
                if api_str == '':
                    ...
                    if ret['succeeded']:
                        self.log_info(self.prompt + 'SUCCESS')
                    else:
                        self.log_info(self.prompt + 'FAILURE')
                        self.log_error(self.prompt + ret['error'])
