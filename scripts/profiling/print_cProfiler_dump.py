...
def main(args):
    ...
    if args.out_filename:
        if args.out_filename == 'default':
            args.out_filename = os.path.splitext(args.dump_filename)[0]+'.txt'
        out_stream = open(args.out_filename, 'w')
    else:
        out_stream = sys.stdout

    ps = pstats.Stats(args.dump_filename, stream=out_stream)
    ps.strip_dirs().sort_stats(args.sort_key).print_stats(args.num_of_lines)
...
if __name__ == '__main__':
    ...
    parser.add_argument('dump_filename',
                        help="dump file generated by cProfiler::dump_stats()")
