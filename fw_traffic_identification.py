...
class FwTrafficIdentifications:
    ...
    def _add_traffic_id(self, dict, key, traffic_id):

        values = dict.get(key, set())
        values.add(traffic_id)
        dict[key] = values

    def _remove_traffic_id(self, dict, key, traffic_id):
    ...
    def add_traffic_identification(self, traffic):
        ...
        if category:
            self._add_traffic_id(self.category_map, category, traffic_id)
        if traffic_class:
            self._add_traffic_id(self.traffic_class_map, traffic_class, traffic_id)
        if importance:
            self._add_traffic_id(self.importance_map, importance, traffic_id)
    ...
    def remove_traffic_identification(self, traffic):
        ...
        if category:
            self._remove_traffic_id(self.category_map, category, traffic_id)
        if traffic_class:
            self._remove_traffic_id(self.traffic_class_map, traffic_class, traffic_id)
        if importance:
            self._remove_traffic_id(self.importance_map, importance, traffic_id)
    ...
    def get_traffic_rules(self, traffic_id, category, traffic_class, importance):
        ...
        for id in traffic_id_set:
            traffic = self.traffic_id_map.get(id)
