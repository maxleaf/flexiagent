...
def vpp_is_configured(config_entities, print_error=True):
    ...
    for (e, amount) in config_entities:
        ...
        if e == 'interfaces':
            ...
            cmd          = r"sudo vppctl sh int addr | grep -E '^(loop|Gigabit|TenGigabit).* \(up\)' | wc -l"  # Don't use 'grep -c'! It exits with failure of not found!
            cmd_on_error = r"sudo vppctl sh int addr | grep -E '^(loop|Gigabit|TenGigabit).* \(up\)'"
