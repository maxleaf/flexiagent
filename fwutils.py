...
def generate_linux_interface_short_name(linux_if_name):
...
def linux_tap_by_interface_name(linux_if_name):
    try:
        links = subprocess.check_output("sudo ip link | grep tap_%s" % generate_linux_interface_short_name(linux_if_name), shell=True)
...
def configure_tap_in_linux_and_vpp(linux_if_name):
    ...
    linux_tap_name = "tap_%s" % generate_linux_interface_short_name(linux_if_name)
