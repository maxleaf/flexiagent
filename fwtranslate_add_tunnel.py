...
def _add_loop_bridge_vxlan(cmd_list, params, loop_cfg, remote_loop_cfg, vxlan_ips, bridge_id, internal, loop_cache_key):
    ...
    if internal:
        ...
        cmd['cmd']['params']  = ["sudo vppctl set ip neighbor loop%d %s %s static" % (bridge_id, remote_loop_ip, remote_loop_mac)]
        cmd['revert'] = {}
        cmd['revert']['name']   = "exec"
        cmd['revert']['descr']  = "delete static arp entry %s %s" % (remote_loop_ip, remote_loop_mac)
        cmd['revert']['params'] = ["sudo vppctl set ip neighbor del loop%d %s %s static" % (bridge_id, remote_loop_ip, remote_loop_mac)]
        cmd_list.append(cmd)
